...
class UI:
    ...
    def draw_wave_info(screen, wave_number):
        ...
        wave_text = ui_font.render(f"WAVE: {wave_number}", True, BLACK)
    ...
    def draw_player_bars(screen, player):
        ...
        bar_width = 200
        bar_height = 20
        pygame.draw.rect(screen, UI_COLORS['hp_bar_bg'],
        ...
        pygame.draw.rect(screen, UI_COLORS['hp_bar_fill'],
        ...
        # Stamina Bar
        pygame.draw.rect(
            screen, UI_COLORS['stamina_bar_bg'], (10, 80, 200, 20))
        st_frac = player.stamina / player.max_stamina
        st_fill = int(200 * max(st_frac, 0))
        pygame.draw.rect(
            screen, UI_COLORS['stamina_bar_fill'], (10, 80, st_fill, 20))
        st_text_str = f"{int(player.stamina)}/{player.max_stamina}"
    ...
    def draw_skill_ui(screen, player, height):
        ...
        for i, skill in enumerate(player.deck.skills):
            ...
            box_width = 100
            box_height = 80
            pygame.draw.rect(screen, UI_COLORS['skill_box_bg'],
    ...
    def draw_skill_cooldown(screen, skill, box_x, box_y, box_width, box_height, now, skill_font):
        if not skill.is_off_cooldown(now):
            ...
            pygame.draw.rect(screen, UI_COLORS['cooldown_overlay'],
