...
class Animation:
    ...
    def __init__(self, name, sprite_sheet_path, sprite_width=32, sprite_height=32, config=None):
        """
        Initializes the animation handler with state configurations.

        Args:
            name (str): Name identifier for this animation.
        ...
        self.current_direction_angle = 90  # For external access
    ...
    def get_default_config(self):
        ...
        return {
            # State: {animations, frames, duration_per_frame, loop?, directional?}
            # Col 1,2
    ...
    def get_state_config(self, state_name):
        """Get configuration for a specific animation state."""
    ...
    def update(self, dt, move_dx=0, move_dy=0):
        ...
        if self.state not in self.config:
            return

        state_cfg = self.config[self.state]
        ...
        if is_directional:
            ...
            if is_moving:
                ...
                self.direction_angle = self._get_closest_direction_angle(
                    movement_angle_rad)
                self.current_direction_angle = self.direction_angle
